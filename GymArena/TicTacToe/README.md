...
This downloads `config.lua`, `startup.lua`, `rename.lua`, `board.lua`, `ai.lua`, `update.lua`, `update_full.lua`, `uninstall.lua`, and installs Basalt2 if it isn't already present. If `config.lua` already exists (e.g. reinstalling after `uninstall.lua` kept it), it's left untouched -- only the other files are refreshed.
