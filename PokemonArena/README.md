...
- Each podium gets a rotating accent color (blue, red, green, repeating for a 3rd+ podium) shown as a 3-row-thick colored bar where the podium used to just say "Left"/"Right" -- the trainer's name is still shown right below it, tinted the same accent color, so color and name work together instead of one replacing the other.
- With **exactly 2 podiums**, the two columns sit edge-to-edge (no reserved center gap) and a "V" is tucked into the right end of podium 1's colored header bar, with "S" tucked into the left end of podium 2's header bar (both black text, spanning the full 3-row header height) -- the two letters sit flush against each other at the seam, reading "VS" without a disruptive marker floating in the middle of the arena (an earlier version reserved a whole center gutter for a "VS" label; that's gone). With 3+ podiums (e.g. a "Middle" podium added in `locations.lua` for a triple battle) there's no single seam to put it at, so no V/S is shown.
- The green winner banner at the bottom of each podium card is also 3 rows thick, matching the header, so the card reads visually balanced top-to-bottom instead of only the header standing out.
