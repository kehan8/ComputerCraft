...
- With **exactly 2 podiums** (and a wide enough screen -- each podium column needs to be at least 11 characters wide), the two columns sit edge-to-edge (no reserved center gap) and a hand-drawn pixel-art "V"/"S" pair is drawn flush against the seam between the two colored header bars, right edge of the V against the left edge of the S -- built the same way as GymArena/TicTacToe's X/O icons (an 11x5 bitmap, one small black square per filled pixel, layered on top of the header's accent color), not text, so the letters can actually be big and legible instead of a single stretched character. With 3+ podiums (e.g. a "Middle" podium added in `locations.lua` for a triple battle), or a screen too narrow for the 11-column pattern, there's no single seam to put it at, so no V/S is shown.
